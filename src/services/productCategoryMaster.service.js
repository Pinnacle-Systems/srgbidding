...
async function getOne(id) {
    const childRecord = await prisma.product.count({ where: { productCategoryId: parseInt(id) } });
    // const childRecord = 0;
    ...
    if (!data) return NoRecordFound("ProductCategory");
...
async function update(id, body) {
    ...
    const dataFound = await prisma.productCategory.findUnique({
        where: {
            id: parseInt(id)
        }
    })
    if (!dataFound) return NoRecordFound("ProductCategory");
