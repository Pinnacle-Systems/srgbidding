...
export const getDateFromDateTimeDB = (dateTime) =>
...
export async function getStockProperty(
  itemType,
  item,
  property,
  storeId,
...
export function getPriceReturnStockQtyWithWeightedAverage(stock, poItem) {
  ...
  return getPriceAddStockQtyWithWeightedAverage(
    { price: oldStockPrice, qty: oldStockQty },
    { price: parseFloat(poItem.price), inwardQty: currentInwardQty },
  );
}
...
export const createManyStockWithId = async (
  tx,
  poType,
  inwardOrReturn,
  items,
  ...
  let promises = items.map(
    async (item) =>
      await stockCreate(tx, poType, inwardOrReturn, item, storeId, branchId),
  );
...
export const stockUpdate = async (tx, poType, inwardOrReturn, item) => {
  ...
  const itemDetails = {
    itemType: poType,
    inOrOut: inwardOrReturn,
    qty: parseFloat(
      inwardOrReturn.includes("Inward")
        ? item.inwardQty
        : substract(0, item.returnQty),
    ),
...
export const updateManyStockWithId = async (
  tx,
  poType,
  inwardOrReturn,
  items,
  ...
  let promises = items.map(
    async (item) => await stockUpdate(tx, poType, inwardOrReturn, item),
  );
  return Promise.all(promises);
};

export const balanceQtyCalculation = (
  poQty,
  cancelQty,
  inwardQty,
  returnQty,
  ...
  return substract(
    substract(
      poQty ? parseFloat(poQty) : 0,
      cancelQty ? parseFloat(cancelQty) : 0,
    ),
    substract(
      inwardQty ? parseFloat(inwardQty) : 0,
      returnQty ? parseFloat(returnQty) : 0,
    ),
  );
};

export const inwardQtyCalculation = (
  poQty,
  cancelQty,
  inwardQty,
  returnQty,
