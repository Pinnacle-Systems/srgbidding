...
import { currencySymbolList, statusDropdown } from "../../../Utils/DropdownData";
...
import { useAddCurrencyMutation, useDeleteCurrencyMutation, useGetCurrenciesQuery, useGetCurrencyByIdQuery, useUpdateCurrencyMutation } from "../../../redux/services/CurrencyMasterService";
...
export default function Form({ onSuccess, onClose, editId, deleteId, deleteLabel } = {}) {
    ...
    const {
        ...
        isLoading,
        isFetching,
    } = useGetCurrenciesQuery({ params, searchParams: searchValue });
    const {
        data: singleData,
        isFetching: isSingleFetching,
        isLoading: isSingleLoading,
    } = useGetCurrencyByIdQuery(id, { skip: !id });

    const [addData] = useAddCurrencyMutation();
    const [updateData] = useUpdateCurrencyMutation();
    const [removeData] = useDeleteCurrencyMutation();
    ...
    const validateData = (data) => {
        if (data.name && data.code && data.symbol) {
            return true;
        }
        return false;
    };
    ...
    useEffect(() => {
        if ((form || onSuccess) && currencyNameRef.current) {
            currencyNameRef.current.focus();
        }
    }, [form, onSuccess]);
        ...
        return (
            <div className="flex flex-col bg-gray-200 min-h-[250px]">
                <div className="border-b py-2 px-4 mx-3 flex mt-4 justify-between items-center sticky top-0 z-10 bg-white">
                    <h2 className="text-lg px-2 py-0.5 font-semibold text-gray-800">Delete Currency</h2>
                </div>
                        ...
                        <>
                            <div className="flex flex-col items-center gap-2">
                                ...
                                <p className="text-xs text-gray-600 text-center">
                                    ...
                                    Remove them first before deleting this Currency.
                                </p>
                            </div>
    ...
    return (
        <div onKeyDown={handleKeyDown} className="p-1">
            <div className="w-full flex bg-white p-1 justify-between  items-center">
                <h5 className="text-lg font-bold text-gray-800">Currency Master</h5>
