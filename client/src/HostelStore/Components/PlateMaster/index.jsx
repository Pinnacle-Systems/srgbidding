...
export default function Form({ onSuccess, onClose, editId, deleteId, deleteLabel } = {}) {
    ...
    const {
        ...
        isLoading,
        isFetching,
    } = useGetPlateMasterQuery({ params, searchParams: searchValue });
    const {
        data: singleData,
        isFetching: isSingleFetching,
        isLoading: isSingleLoading,
    } = useGetPlateMasterByIdQuery(id, { skip: !id });
    const [trigger, { data: LazyData }] = useLazyGetPlateMasterByIdQuery();
    ...
    const saveData = (nextProcess) => {
        ...
        if (foundItem) {
            Swal.fire({
                text: "The Plate Name already exists.",
                icon: "warning",
                didClose: () => {
        ...
        return (
            <div className="min-h-[250px] flex flex-col bg-gray-200">
                <div className="border-b py-2 px-4 mx-3 mt-4 bg-white">
                    <h2 className="text-lg font-semibold">Delete Plate</h2>
                </div>
            ...
            <div onKeyDown={handleKeyDown} className="h-full flex flex-col bg-gray-200">
                <div className="border-b py-2 px-4 mx-3 flex mt-4 justify-between items-center sticky top-0 z-10 bg-white">
                    <h2 className="text-lg px-2 py-0.5 font-semibold text-gray-800">
                        {editId ? "Edit Plate" : "Add New Plate"}
                    </h2>
                    <button
                        type="button"
