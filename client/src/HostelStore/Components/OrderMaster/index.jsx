...
import { useAddOrderMasterMutation, useDeleteOrderMasterMutation, useGetOrderMasterByIdQuery, useGetOrderMasterQuery, useUpdateOrderMasterMutation } from "../../../redux/services/OrderMasterService";
...
export default function Form({ onSuccess, defaultName = "" }) {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useGetOrderMasterQuery({ params, searchParams: searchValue });
  const {
    data: singleData,
    isFetching: isSingleFetching,
    isLoading: isSingleLoading,
  } = useGetOrderMasterByIdQuery(id, { skip: !id });

  const [addData] = useAddOrderMasterMutation();
  const [updateData] = useUpdateOrderMasterMutation();
  const [removeData] = useDeleteOrderMasterMutation();
  ...
  const saveData = (nextProcess) => {
    ...
    if (id) {
      foundItem = allData?.data
        ?.filter((i) => i.id != id)
        ?.some((item) => item.docId === name);
    } else {
      foundItem = allData?.data?.some((item) => item.docId === name);
    }

    if (foundItem) {
      Swal.fire({
        text: "The Order No already exists.",
        icon: "warning",
        didClose: () => {
          countryNameRef?.current?.focus();
        }
  ...
  return (
    <div onKeyDown={handleKeyDown} className="p-1 h-[87%]">
      <div className="w-full flex bg-white p-1 justify-between  items-center">
        <h5 className="text-lg font-bold text-gray-800">Order Master</h5>
